...
| 8. Prove authentication, isolation, and runner compatibility | #239 | Repository proof merged; hosted acceptance blocked | Steps 4-7 (#235, #236, #237, #238; merged) and completed development setup Step 1 (#232) | PR #256 (merged, non-closing); follow-up PR pending | `10c246d` | Credential-free two-user ownership, machine-token, legacy-claim, signed-out, auth-surface, and artifact-redaction evidence passed checker cycle 2, exact-head CI (776 unit and 14 E2E tests), security review, and maintainer-agent approval. The linked development Clerk setup and sanitized doctor gate from #232 are complete. The opt-in `bun run test:e2e:clerk` harness now bootstraps Clerk setup in the launcher process, requires distinct `+clerk_test` identities, and binds each context to its resolved primary email; it has not run against the linked instance. | Issue #239 remains open for hosted browser/provider evidence and safely isolated runner-backed full `bun run verify` evidence; Google/Apple remain operator-evidence paths because the official helper does not provide deterministic OAuth automation. |
