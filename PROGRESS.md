...
| 3. Resolve Clerk and development identities to internal users | #234 | Implemented; checker passed; draft PR | None | Draft PR #247 | `555e44b`, `9edfc59`, `767ba87` | Migration 0014 adds only nullable unique `users.clerk_user_id`; the request resolver supports shared development identity and typed Clerk `401` results, while the server adapter awaits `auth()` and passes only `userId`. Isolated migration and focused schema/resolver/claim/adapter/CLI tests passed (4 files, 57 tests); separate-connection race suites passed five repeated runs (3 files, 29 tests each); the previously timed-out unrelated start-route test passed three targeted runs; full unit passed after merged #242 (70 files, 594 tests); format, lint, typecheck, build, current `test:e2e:ci` (14 tests), migration-lineage, diff, and secret checks passed. No production claim, real-provider E2E, or provider/secret mutation occurred. | Draft PR #247 remains under coordinator control. Recheck the rebased head and GitHub checks before any readiness or review transition; never run the legacy claim against production without separate approval. |
...
- 2026-07-10: Issue #234 implemented the nullable unique Clerk identity,
  ...
  draft PR #247 was rebased onto merged #242, and current isolated database,
  repeated race, full unit, build, and `test:e2e:ci` gates passed. No production
  claim was executed and no review/readiness state is claimed here.
