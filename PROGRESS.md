...
| 11 | PlanetScale release-database connection compatibility | Run [`30965649983`](https://github.com/ametel01/plingpling/actions/runs/30965649983) published and scanned `756aae3badf34b68b53c275d333ceecd4c87da3f@sha256:2a35eb009d0ecc2dfa0f50f516a21bf92fe69d72999b40fc4677580bf59902e6`, then staged its no-domain control plane successfully. The canary failed in 1.2 seconds before its first database insert because the JavaScript Postgres driver forwarded PlanetScale's `sslrootcert=system` URL parameter as an unsupported server startup setting (`42704`). Read-only inspection through the same dedicated URL, with only that parameter removed, proved the schema is current and contained zero canary runner or provisioning-event rows; DigitalOcean was never contacted. The shared client now removes only `sslrootcert=system` while preserving `sslmode=verify-full`, explicit custom certificate settings, credentials, and other query parameters. The unchanged dedicated URL then passed a read-only connection replay. Frozen install, 3 focused files / 18 tests, format/lint across 385 files, typecheck, clean migrations through 0024, 165 files / 1,582 tests, production build, and diff hygiene passed. Production promotion was skipped and no Droplet was created. A new authorized run is required; no live acceptance is claimed. |
...
- 2026-08-05: The first authorized release attempt, workflow run
  [`30959832776`](https://github.com/ametel01/plingpling/actions/runs/30959832776),
  failed in two seconds while GitHub prepared the publish job because the Trivy
  action reference omitted its required `v` tag prefix. All downstream jobs
  were skipped, so no image, deployment, provider request, or Droplet was
  created. Corrected both runner and Hermes publication workflows to the
  existing `aquasecurity/trivy-action@v0.28.0` tag and strengthened the release
  source contract to reject the unprefixed revision. Frozen install, 8 focused
...
- 2026-08-05: Release run
  [`30962185226`](https://github.com/ametel01/plingpling/actions/runs/30962185226)
  passed the image build and publication, immutable digest verification, Trivy
  critical-fixable gate, and SARIF artifact upload for candidate
  `9e61f46c344cf35049db35f088242747abbeb1b3@sha256:c8b8bfe9261cfe06e7127122dd14c516c1f57648fec295655486a3836fe157de`.
  The next job failed before staging while pulling production project settings
  because the GitHub production environment's direct Vercel project targeting
  could not be resolved. Refreshed the two project identifier secrets from the
  verified local link, and verified the environment-token path against the exact
  account, team, and project without printing the token. Removed all Vercel
  token command-line arguments so the CLI consumes the existing
  environment-scoped token without exposing it in process listings. The staging
  deploy, canary, provider access, and production
  deployment were skipped, so no Vercel deployment or Droplet occurred. Frozen
  install, 10 focused tests, format, lint, typecheck, clean migrations, all
  1,579 tests, production build, YAML parsing, authenticated read-only Vercel
...
- 2026-08-05: Release run
  [`30965649983`](https://github.com/ametel01/plingpling/actions/runs/30965649983)
  published and scanned candidate
  `756aae3badf34b68b53c275d333ceecd4c87da3f@sha256:2a35eb009d0ecc2dfa0f50f516a21bf92fe69d72999b40fc4677580bf59902e6`
  and created its no-domain Vercel staging deployment. The canary then
  returned `smoke_failed` in 1.2 seconds with verified cleanup. Read-only replay
  identified SQLSTATE `42704`: the JavaScript Postgres driver forwarded
  PlanetScale's `sslrootcert=system` URL parameter as an unsupported server
  startup setting. Removing only that parameter connected successfully while
  preserving `sslmode=verify-full`; the release database had a current schema
  and zero canary runner or provisioning-event rows, proving DigitalOcean was
  never contacted. Normalized this official PlanetScale URL form at the shared
  database-client boundary and preserved explicit custom root certificate
  settings. The unchanged dedicated URL passed a read-only shared-client replay.
  Frozen install, 3 focused files / 18 tests, format/lint across 385 files,
  typecheck, clean migrations through 0024, 165 files / 1,582 tests, production
