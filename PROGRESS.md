...
| 7. Preserve full registration-free development access | #238 | Implemented; PR open; tracker review fix pending | Steps 2 and 3 (merged #233/PR #244 and #234/PR #247) | PR #251 (open) | `d077a83` (reviewed implementation head) | A server-only `AGENTBAY_AUTH_MODE` policy now gives non-Vercel loopback environments registration-free shared-user access, requires complete Clerk configuration for production/custom hosts, permits preview development only with an explicit exact protection attestation, ignores caller-controlled request hosts, preserves the Basic barrier and exact runner-machine bypasses, and validates Vercel builds before spawning commands. At `d077a83`, the independent checker passed 8 focused files / 141 tests plus 34 adversarial environment cases, format, lint, typecheck, build, and diff checks; exact-head CI run 29081022680 passed migration, 73 files / 668 unit tests, build, and `test:e2e:ci` 14/14, with GitGuardian and Socket checks green. | PR #251 is open and not merged. Land this tracker-only review fix, then rerun exact-head checker/CI, refresh the PR body, and obtain maintainer re-review. No hosted Vercel protection probe, deployment, env mutation, Clerk/provider operation, secret use, or provider-backed full E2E was performed. |
