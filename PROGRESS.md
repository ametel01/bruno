...
Step 6 cycle-2 repair is complete and awaiting independent recheck. Projected
credential reads now reject FIFO and other nonregular substitutions before a
nonblocking no-follow open, verify pre/post file identity, and recover cleanly;
focused status/canary tests prove bounded safe behavior with zero probe calls.
Repair validation passed 108 files / 996 tests, local smoke in 42,165 ms,
production build, 14 E2E tests, and fail-closed staging with no side effects.
Step 7 remains blocked until checker cycle 2 accepts the repair.
...
| 6. Split Runner Launch Acceptance From Observed Readiness | Repair complete; checker cycle 2 pending | Steps 0-5 | `4e2897a` plus this repair commit | Product commit added strict runner launch/status/canary contracts; one request-scoped 30-second launch budget; cancellation-safe cleanup; deterministic exact reuse/replacement; bounded private health/canary seams; stopped/cancelled snapshots; accepted/ready compatibility DTOs; and stale-result lifecycle guards. Initial combined focused coverage passed 11 files / 216 tests. Checker cycle 1 otherwise passed 7 files / 207 focused tests, local smoke, 108 files / 995 full tests, build, 14 E2E tests, and fail-closed staging, but reproduced a FIFO `.env` blocking open. Cycle 2 pre-lstats nonregular/hardlinked/oversized files, opens with no-follow plus nonblocking flags, verifies post-open device/inode identity, closes every handle, and adds a bounded FIFO status/canary regression with zero probe calls and regular-file recovery; focused Step 5/6 coverage passed 4 files / 61 tests. | Independent checker cycle 2 is next; Step 7 remains blocked until acceptance. |
...
- No implementation blocker remains for Step 6; checker cycle 2 is next.
