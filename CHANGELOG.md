...
- Runner releases are now SHA-only immutable candidates that must pass digest verification, critical-vulnerability scanning, and one explicitly authorized, serialized disposable release canary before the exact tested commit and digest can reach production; test processes cannot construct a live DigitalOcean client, linked-repository production builds cannot bypass the canary, rollout is one runner per reconciliation, and artifact-backed rollback halts further fleet work.
