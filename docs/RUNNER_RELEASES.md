...
The first synthetic-container create uses a bounded 2.5-second stabilization window after an error.
Before each retry, the runner reconciles the exact fixture name and ownership label, removes any
ambiguous create result, and verifies that no matching container remains.
