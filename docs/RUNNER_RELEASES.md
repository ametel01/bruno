...
The first local image lookup and synthetic-container create use bounded 2.5-second stabilization
windows after an error. Before each create retry, the runner reconciles the exact fixture name and
ownership label, removes any ambiguous create result, and verifies that no matching container
remains.
