...
Every record digest and the aggregate digest must be distinct. Missing, duplicate, malformed,
...
and sanitized. Supply `mode: release`, the exact `runtime_revision`, and the allowlisted JSON as
...
An absent or invalid production-provider summary produces `outcome: "denied"`. Do not retry a failed
