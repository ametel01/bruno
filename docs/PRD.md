# AgentBay Hermes Fleet Control Plane PRD
...
Build AgentBay as a desktop-first web SaaS for managing persistent Hermes agents, with a mobile-responsive control panel for monitoring, approvals, pause/resume, alerts, and quick recovery. The product should feel closer to Vercel, Railway, DigitalOcean, and Datadog for AI agents than to a chat app.
...
1. As a solo founder, I want to create an AgentBay account, so that I can manage persistent AI agents from one dashboard.
...
8. As a developer, I want to create a GitHub Issue Agent template, so that repository maintenance can be supervised from AgentBay.
...
43. As a security-conscious user, I want runner API requests to be authenticated, so that only AgentBay can control my runner.
...
55. As a business owner, I want subscription status to sync into AgentBay, so that cancelled or unpaid users cannot create new paid resources.
...
- Treat AgentBay as an agent operations control plane, not a commodity Hermes hosting wrapper.
