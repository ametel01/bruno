...
- [ ] If `PROGRESS.md` or another durable progress document requires a commit reference, the builder or coordinator must refresh placeholder wording after an implementation commit, PR head commit, or replacement commit exists.
...
Use this section only when `PROGRESS.md` or another durable progress document requires a commit reference. Do not require exact commit references for issues that do not ask for them.
