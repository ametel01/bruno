...
describe("Founder production provider qualification", () => {
  it("accepts only independently bound Clerk production, Lemon test-mode, and live-canary evidence", () => {
    ...
    expect(
      evaluateFounderProductionProviderQualification({
        ...
        liveTargetAuthority: LIVE_TARGET_AUTHORITY,
      }),
    ).toEqual([]);
  });
  ...
  it("fails closed without protected target authority and rejects self-consistent wrong targets", () => {
    const value = summaryValue();
    const liveCanary = qualification(value, 2);
    liveCanary.intendedStoreDigest = `sha256:${"d".repeat(64)}`;
    liveCanary.observedStoreDigest = liveCanary.intendedStoreDigest;
    liveCanary.intendedProductDigest = `sha256:${"e".repeat(64)}`;
    liveCanary.observedProductDigest = liveCanary.intendedProductDigest;
    const summary = parse(value);
    ...
    ).toEqual(["lemon_squeezy_live_target_authority_missing"]);
    expect(evaluate(summary)).toEqual([
      "lemon_squeezy_live_store_mismatch",
      "lemon_squeezy_live_product_mismatch",
    ]);
  });
